...
class Rectangle(
        ...
        fileName: String = "dougsRectangle3_.thr"
               ) :
        Shape(
    ...
    override fun defineSegments() { // hmm, pass in delta rho and theta here adn you've got a constructor...
        //        val point0 = Point.pointFromDeg(rho = 1.0, thetaInDegrees = 45.0)// 90 +/- 45
        //        val point1 = Point.pointFromDeg(rho = 1.0, thetaInDegrees = 135.0)
        //        val point2 = Point.pointFromDeg(rho = 1.0, thetaInDegrees = 225.0)//270 +/- 45
        //        val point3 = Point.pointFromDeg(rho = 1.0, thetaInDegrees = 315.0// notice for this shape we must go to staring point as a positive rotation
        //        val point4 = Point.pointFromDeg(rho = 1.0, thetaInDegrees = 405.0) // so we go past 360 and don't go back to 0
        val point0 = Point.pointFromDeg(rho = 1.0, thetaInDegrees = 70.0) // 90 +/- 20
        val point1 = Point.pointFromDeg(rho = 1.0, thetaInDegrees = 110.0)
        val point2 = Point.pointFromDeg(rho = 1.0, thetaInDegrees = 250.0) //270 +/- 20
        val point3 = Point.pointFromDeg(rho = 1.0, thetaInDegrees = 290.0) // notice for this shape we must go to staring point as a positive rotation
        val point4 = Point.pointFromDeg(rho = 1.0, thetaInDegrees = 360.0 + 70.0) // so we go past 360 and don't go back to 0
    ...
    override fun withOffset(deltaRho: Double, deltaTheta: Double, iteration: Int): Shape {
        return Rectangle(numberOfCopiesPerRev,
                         rhoRemainingPerRev,
                         numberOfRevs,
