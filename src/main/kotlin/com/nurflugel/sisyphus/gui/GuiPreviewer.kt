...
class GuiPreviewer {
  ...
  internal fun initialize() {
    ...
    frame.title = "Click any key to close" //    frame.rootPane.border = BorderFactory.createEmptyBorder();
  ...
  fun showPreview(trackFileName: String, lines: MutableList<String>, showNames: Boolean) { // initial point of null
    ...
    frame.title = "$trackFileName     Click any key to close" // todo reset the values for scales based on the present GUI size
  ...
  private fun drawUiPreview(
    ...
    graphics.draw(line)
    if (showName) { //            graphics.setRenderingHints(nonAliasedRenderingHints)
      graphics.drawString(fileName, 20, 30); //            graphics.setRenderingHints(aliasedRenderingHints)
    }
  }
}
